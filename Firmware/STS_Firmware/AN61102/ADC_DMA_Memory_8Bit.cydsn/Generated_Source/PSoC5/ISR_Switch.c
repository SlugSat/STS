...
void ISR_Switch_Stop(void)
{
    ...
    ISR_Switch_SetVector(&IntDefaultHandler);
}
...
void ISR_Switch_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)ISR_Switch__INTC_NUMBER] = address;
}
...
cyisraddress ISR_Switch_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)ISR_Switch__INTC_NUMBER];
}
...
*
*   Note calling ISR_Switch_Start or ISR_Switch_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after ISR_Switch_Start or ISR_Switch_StartEx has been called. 
...
uint8 ISR_Switch_GetState(void)
{
    ...
    return ((*ISR_Switch_INTC_SET_EN & (uint32)ISR_Switch__INTC_MASK) != 0u) ? 1u:0u;
}
